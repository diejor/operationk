...
namespace input {
    ...
    inline string get_file_name_validated() {
        string input_file_name = line("Enter file name: ");

        while (!extensions::has(input_file_name)) {
            output::error("invalid file extension");
            input_file_name = line("Enter file name again: ");
        }
     ...
     */
    inline ifstream try_open_input(string file_name) {
        ifstream file(file_name);
        while (!file.is_open()) {
            output::error("file does not exist");
            file_name = line("Enter file name again: ");
            file.open(file_name);
        }
        return file;
    }
     ...
     */
    inline ifstream get_input() {
        ifstream file;
        string file_name = get_file_name_validated();
        file = try_open_input(file_name);
